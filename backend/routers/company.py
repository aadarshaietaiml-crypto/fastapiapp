...
def get_all_company(db:Session=Depends(get_db),current_user = Depends (role_required(get_current_user))):
    companies=db.query(Company).all()
    return companies

@router.get("/{company_id}",status_code=status.HTTP_200_OK,response_model=CompanyResponse)
def get_company(company_id: int,db:Session=Depends(get_db),current_user = Depends (role_required(get_current_user))):
